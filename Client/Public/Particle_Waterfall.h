...
class CParticle_Waterfall final : public CGameObject
{
	using Super = CGameObject;
private:
	/* 원형을 생성할 때 */
	CParticle_Waterfall(ID3D11Device* pDevice, ID3D11DeviceContext* pContext);
	/* 사본을 생성할 때 */
	CParticle_Waterfall(const CParticle_Waterfall& rhs); /* 복사 생성자. */
	virtual ~CParticle_Waterfall() = default;
...
public:
	static	CParticle_Waterfall* Create(ID3D11Device* pDevice, ID3D11DeviceContext* pContext);
