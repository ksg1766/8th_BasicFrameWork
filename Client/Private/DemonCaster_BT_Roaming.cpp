...
CDemonCaster_BT_Roaming::CDemonCaster_BT_Roaming()
{
}

void CDemonCaster_BT_Roaming::OnStart()
{
	Super::OnStart(0);
	SetRoamingPoint();
}

CBT_Node::BT_RETURN CDemonCaster_BT_Roaming::OnUpdate(const _float& fTimeDelta)
{
...
void CDemonCaster_BT_Roaming::OnEnd()
{
	AbortRoaming();
	Super::OnEnd();
}

void CDemonCaster_BT_Roaming::ConditionalAbort(const _float& fTimeDelta)
{
}

void CDemonCaster_BT_Roaming::SetRoamingPoint()
{
	BLACKBOARD& hashBlackBoard = m_pBehaviorTree->GetBlackBoard();
	const auto& tRoamingPoint = hashBlackBoard.find(TEXT("RoamingPoint"));
...
void CDemonCaster_BT_Roaming::AbortRoaming()
{
	BLACKBOARD& hashBlackBoard = m_pBehaviorTree->GetBlackBoard();
	const auto& tRoamingPoint = hashBlackBoard.find(TEXT("RoamingPoint"));
...
_bool CDemonCaster_BT_Roaming::IsAggro()
{
...
_bool CDemonCaster_BT_Roaming::IsZeroHP()
{
...
CDemonCaster_BT_Roaming* CDemonCaster_BT_Roaming::Create(CGameObject* pGameObject, CBehaviorTree* pBehaviorTree, const BEHAVEANIMS& tBehaveAnim, CMonoBehaviour* pController)
{
	CDemonCaster_BT_Roaming* pInstance = new CDemonCaster_BT_Roaming;

	if (FAILED(pInstance->Initialize(pGameObject, pBehaviorTree, tBehaveAnim, pController)))
	{
		MSG_BOX("Failed to Created : CDemonCaster_BT_Roaming");
...
void CDemonCaster_BT_Roaming::Free()
{
	Super::Free();
}